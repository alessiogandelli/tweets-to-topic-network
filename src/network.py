...
def create_network(df_tweets):
    ...
    topics = df_labeled['topics'].to_dict()
    ...
    men_edges = [(i, m) for i in df_tweets.index for m in df_tweets.loc[i]['mentions_name']] #mentions
    ...
    # set bipartite = 0 ( so actors) to the new nodes added 
    for i in g.nodes:
        if 'bipartite' not in g.nodes[i]:
...
g,x,t = create_network(df_labeled)
